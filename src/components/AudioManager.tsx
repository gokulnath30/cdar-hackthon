...
export function AudioManager(props: { transcriber: Transcriber }) {
    ...
    useEffect(() => {
     ...
     useEffect(() => {
        const handler = (e: Event) => {
            const ce = e as CustomEvent<{ blob: Blob }>;
            if (ce.detail?.blob) {
                // Load the recorded audio; pass true for autoStart if desired
                setAudioFromRecording(ce.detail.blob, true); // true => immediately start transcription
            }
        };
        window.addEventListener("bottomnav:recording-complete", handler);
        return () => window.removeEventListener("bottomnav:recording-complete", handler);
    }, []);
    ...
    const setAudioFromRecording = async (data: Blob, autoStart: boolean = false) => {
        resetAudio();
        setProgress(0);
